// v19 - Limpa campos do treino atual sem apagar o peso anterior.
// Carrega historico do Supabase e preserva a referencia do ultimo treino por exercicio.
(function(){
  if(!window.TRAINING_DATA || !window.TRAINING_DATA.academia) return;

  var TABLE = 'exercicios_registros';
  var VERSION_LABEL = 'v19';
  var CFG = {
  ...
  function sortHistory(key){
    ...
    state.exerciseHistory[key].sort(function(a,b){return new Date(b.created_at||0)-new Date(a.created_at||0);});
  }

  function prevRow(key){
    sortHistory(key);
    var h=state.exerciseHistory[key] || [];
    for(var i=0;i<h.length;i++){
      if(h[i].peso_kg!==null && h[i].peso_kg!==undefined && h[i].peso_kg!=='') return h[i];
    }
    return h[0] || null;
  }
  ...
  function prev(key){
    var p=prevRow(key);
    if(!p) return '<strong>-</strong><small>Sem registro anterior</small>';
    var peso = (p.peso_kg!==null && p.peso_kg!==undefined && p.peso_kg!=='') ? p.peso_kg+' kg' : '-';
    var series = p.series ? p.series+' séries' : '';
    var reps = p.repeticoes ? ' • '+p.repeticoes+' reps' : '';
  ...
  async function loadHistory(){
    ...
    try{
      var res = await supabaseClient.from(TABLE).select('*').eq('device_id',CLOUD_DEVICE_ID).order('created_at',{ascending:false}).limit(500);
      if(res.error) throw res.error;
      var grouped = {};
      (res.data || []).forEach(function(row){
        var key=row.exercicio_key || normalize(row.exercicio||'');
        if(!grouped[key]) grouped[key]=[];
        grouped[key].push(row);
      });
      Object.keys(grouped).forEach(function(key){
        state.exerciseHistory[key]=grouped[key];
        sortHistory(key);
      });
      save();
    }catch(e){
      // Mantem historico local se a nuvem nao responder.
    }
  }
  ...
  function card(row,letter){
    ...
    var doneKey='gym-v19-'+letter+'-'+key;
    var d=draft(key);
    ...
    return '<article class="exercise-card '+(done?'done':'')+'" data-exercise-key="'+key+'" data-done-key="'+doneKey+'">'+
      ...
      '<div class="exercise-body">'+
        ...
        '<div class="weight-box weight-box-v10">'+
          ...
          '<div class="previous-weight" data-prev-key="'+key+'"><span>Peso anterior</span>'+prev(key)+'</div>'+ 
        '</div>'+ 
  ...
  function home(){
    ...
    el.innerHTML='<section class="v9-training-shell"><div class="v9-head card"><p class="eyebrow">Treinos de academia</p><h2>Escolha o treino</h2><p class="muted">v19: campos em branco após envio, mantendo peso anterior.</p></div><div class="training-home-grid">'+
      ['A','B','C'].map(function(l){var c=CFG[l]; return '<button type="button" class="training-home-card" data-open-v19="'+l+'"><div class="home-icon">'+c.icon+'</div><div><strong>'+c.title+'</strong><span>'+c.subtitle+'</span><small>'+rows(l).length+' exercícios</small></div></button>';}).join('')+
      '</div></section>';
  }
  ...
  function detail(letter){
    ...
    el.innerHTML='<section class="v9-training-shell"><button class="back-training" type="button" data-back-v19="1">← Voltar aos treinos</button><div class="card workout-hero"><div class="home-icon large">'+c.icon+'</div><div><p class="eyebrow">'+c.title+'</p><h2>'+c.subtitle+'</h2><p class="muted">'+c.desc+'</p></div></div><div class="exercise-list">'+list.map(function(r){return card(r,letter);}).join('')+'</div><div class="card workout-complete-box"><h2>Finalizar treino</h2><p class="muted">Ao concluir, o lançamento atual será limpo. O peso anterior continuará visível como referência.</p><button type="button" class="primary" data-complete-v19="'+letter+'">Concluir treino e enviar</button><div class="exercise-save-status"></div></div></section>';
  }

  function payload(row,letter){
    var key=normalize(row['Exercício']||''); var d=draft(key);
    return {device_id:CLOUD_DEVICE_ID,semana:week(),treino_tipo:letter,exercicio:row['Exercício']||'',exercicio_key:key,peso_kg:n(d.peso),series:d.series===''?null:Number(d.series),repeticoes:d.reps===''?null:Number(d.reps),observacoes:d.obs||null,dados_json:{treino:row['Treino']||'',series_planejadas:row['Séries x reps']||'',intensidade:row['Intensidade']||'',feito:!!state.gymDone['gym-v19-'+letter+'-'+key],app_version:VERSION_LABEL}};
  }
  ...
  function clearCurrentInputsOnly(letter){
    ...
    el.querySelectorAll('.exercise-card').forEach(function(cardEl){
      cardEl.classList.remove('done');
      var st=cardEl.querySelector('.exercise-status');
      if(st) st.textContent='Em aberto';
    });
    el.querySelectorAll('.previous-weight[data-prev-key]').forEach(function(box){
      var key=box.getAttribute('data-prev-key');
      box.innerHTML='<span>Peso anterior</span>'+prev(key);
    });
  }
  ...
  async function complete(letter,btn){
    ...
    try{
      if(cloudReady && supabaseClient){
        ...
        clearCurrentInputsOnly(letter);
        box.textContent='Treino enviado. Campos limpos; peso anterior mantido.';
        box.className='exercise-save-status ok visible';
      } else {
        addToHistory(dataRows.map(function(r){return Object.assign({local:true},r);}));
        clearCurrentInputsOnly(letter);
        box.textContent='Treino salvo localmente. Campos limpos; peso anterior mantido.';
      ...
      addToHistory(dataRows.map(function(r){return Object.assign({local:true},r);}));
      clearCurrentInputsOnly(letter);
      box.textContent='Treino salvo localmente. Campos limpos; peso anterior mantido. Verifique Supabase/RLS.';
      box.className='exercise-save-status warn visible';
    }
  ...
  document.addEventListener('click',function(e){
    var open=e.target.closest('[data-open-v19]'); if(open){detail(open.getAttribute('data-open-v19')); return;}
    if(e.target.closest('[data-back-v19]')){home(); return;}
    var completeBtn=e.target.closest('[data-complete-v19]'); if(completeBtn){complete(completeBtn.getAttribute('data-complete-v19'),completeBtn); return;}
  });
