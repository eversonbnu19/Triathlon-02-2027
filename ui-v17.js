// v17 - Reconstrói a aba Treinos de forma única e robusta.
// Remove conflito das camadas anteriores e garante acesso a Academia A/B/C.
(function(){
  if(!window.TRAINING_DATA || !window.TRAINING_DATA.academia) return;

  var TABLE = 'exercicios_registros';
  var VERSION_LABEL = 'v17';
  var CFG = {
  ...
  function card(row,letter){
    ...
    var doneKey='gym-v17-'+letter+'-'+key;
    var d=draft(key);
  ...
  function home(){
    ...
    el.innerHTML='<section class="v9-training-shell"><div class="v9-head card"><p class="eyebrow">Treinos de academia</p><h2>Escolha o treino</h2><p class="muted">Acesso corrigido na v17. Escolha A, B ou C para abrir os exercícios.</p></div><div class="training-home-grid">'+
      ['A','B','C'].map(function(l){var c=CFG[l]; return '<button type="button" class="training-home-card" data-open-v17="'+l+'"><div class="home-icon">'+c.icon+'</div><div><strong>'+c.title+'</strong><span>'+c.subtitle+'</span><small>'+rows(l).length+' exercícios</small></div></button>';}).join('')+
      '</div></section>';
  }
  ...
  function detail(letter){
    ...
    el.innerHTML='<section class="v9-training-shell"><button class="back-training" type="button" data-back-v17="1">← Voltar aos treinos</button><div class="card workout-hero"><div class="home-icon large">'+c.icon+'</div><div><p class="eyebrow">'+c.title+'</p><h2>'+c.subtitle+'</h2><p class="muted">'+c.desc+'</p></div></div><div class="exercise-list">'+list.map(function(r){return card(r,letter);}).join('')+'</div><div class="card workout-complete-box"><h2>Finalizar treino</h2><p class="muted">Ao concluir, os exercícios preenchidos serão enviados para a base de dados.</p><button type="button" class="primary" data-complete-v17="'+letter+'">Concluir treino e enviar</button><div class="exercise-save-status"></div></div></section>';
  }

  function payload(row,letter){
    var key=normalize(row['Exercício']||''); var d=draft(key);
    return {device_id:CLOUD_DEVICE_ID,semana:week(),treino_tipo:letter,exercicio:row['Exercício']||'',exercicio_key:key,peso_kg:n(d.peso),series:d.series===''?null:Number(d.series),repeticoes:d.reps===''?null:Number(d.reps),observacoes:d.obs||null,dados_json:{treino:row['Treino']||'',series_planejadas:row['Séries x reps']||'',intensidade:row['Intensidade']||'',feito:!!state.gymDone['gym-v17-'+letter+'-'+key],app_version:VERSION_LABEL}};
  }
  ...
  async function complete(letter,btn){
    ...
    try{
      if(cloudReady && supabaseClient){var res=await supabaseClient.from(TABLE).insert(dataRows).select(); if(res.error) throw res.error; box.textContent='Treino enviado. '+dataRows.length+' exercícios registrados.'; box.className='exercise-save-status ok visible';}
      else {box.textContent='Treino salvo localmente. Nuvem indisponível.'; box.className='exercise-save-status warn visible';}
      dataRows.forEach(function(r){var k=r.exercicio_key; if(!state.exerciseHistory[k]) state.exerciseHistory[k]=[]; state.exerciseHistory[k].unshift(Object.assign({created_at:new Date().toISOString(),local:!cloudReady},r));}); save(); renderDashboard();
    }catch(e){box.textContent='Treino salvo localmente. Verifique Supabase/RLS.'; box.className='exercise-save-status warn visible'; save();}
    btn.disabled=false; btn.textContent='Concluir treino e enviar';
  }

  document.addEventListener('click',function(e){
    var open=e.target.closest('[data-open-v17]'); if(open){detail(open.getAttribute('data-open-v17')); return;}
    if(e.target.closest('[data-back-v17]')){home(); return;}
    var completeBtn=e.target.closest('[data-complete-v17]'); if(completeBtn){complete(completeBtn.getAttribute('data-complete-v17'),completeBtn); return;}
  });
